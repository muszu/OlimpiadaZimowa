DROP FUNCTION IF EXISTS najstarsi_medalisci_ind();
CREATE FUNCTION najstarsi_medalisci_ind()
    ...
    FROM (SELECT imie, nazwisko, id_panstwa, age(data_rozgrywki,data_urodzenia) AS wiek, id_dyscypliny, medal FROM dyscypliny_medalisci_ind
        ...
        WHERE age(data_rozgrywki,data_urodzenia) = (SELECT MAX(age(data_rozgrywki,data_urodzenia))
